...
function(oj, ko, $, MessageUtils, ApmConstants)
{
    var DateTimeUtils = (function () 
    {
        ...
        { 
            ...
            {
                if (units === self.apmConstants.timeUnit.NANOSECONDS)
                {
                    if (value > 1000000)
                    {
                        value = value / 1000000;
                        units = self.apmConstants.timeUnit.MILLISECONDS;
                       return self.displayTime(value, units);
                    }
                }
                else if (units === self.apmConstants.timeUnit.MILLISECONDS)
                {
                    if (value > 1000)
                    {
                        value = value / 1000;
                        units = self.apmConstants.timeUnit.SECONDS;
                        return self.displayTime(value, units);
                    }
                }
                else if (units === self.apmConstants.timeUnit.SECONDS)
                {
                    if (value > 60)
                    {
                        value = value / 60;
                        units = self.apmConstants.timeUnit.MINUTES;
                        return self.displayTime(value, units);
                    }
                }
                else if (units === self.apmConstants.timeUnit.MINUTES)
                {
                    if (value > 60)
                    {
                        value = value / 60;
                        units = self.apmConstants.timeUnit.HOURS;
                        return self.displayTime(value, units);
                    }
                }
                else if (units === self.apmConstants.timeUnit.HOURS)
                {
                    if (value > 60)
                    {
                        value = value / 60;
                        units = self.apmConstants.timeUnit.DAYS;
                        return self.displayTime(value, units);
                    }
                } 
            ...
            {
                if (units === self.apmConstants.timeUnit.MILLISECONDS)
                    return MessageUtils.getInstance().getTranslatedString('generalProperties.MS_WITH_VALUE', value);
                else if (units === self.apmConstants.timeUnit.NANOSECONDS)
                ...
                else if (units === self.apmConstants.timeUnit.DAYS)
            ...
            self.timePeriodInfo = [
                         {name: self.apmConstants.timePeriod.LAST15MINUTES, msecs: 15*60*1000, overview: 1, spark: 1}
                        ,{name: self.apmConstants.timePeriod.LAST30MINUTES, msecs: 30*60*1000, overview: 1, spark: 3}
                        ,{name: self.apmConstants.timePeriod.LAST_60MINUTES, msecs: 60*60*1000, overview: 1, spark: 5}
                        ,{name: self.apmConstants.timePeriod.LASTHOUR, msecs: 60*60*1000, overview: 1, spark: 5}
                        ,{name: self.apmConstants.timePeriod.LAST2HOURS, msecs: 2*60*60*1000, overview: 2, spark: 10}
                        ,{name: self.apmConstants.timePeriod.LAST4HOURS, msecs: 4*60*60*1000, overview: 4, spark: 20}
                        ,{name: self.apmConstants.timePeriod.LAST6HOURS, msecs: 6*60*60*1000, overview: 6, spark: 30}
                        ,{name: self.apmConstants.timePeriod.LAST8HOURS, msecs: 8*60*60*1000, overview: 8, spark: 30}
                        ,{name: self.apmConstants.timePeriod.LAST12HOURS, msecs: 12*60*60*1000, overview: 15, spark: 60}
                        ,{name: self.apmConstants.timePeriod.LAST24HOURS, msecs: 24*60*60*1000, overview: 15, spark: 120}
                        ,{name: self.apmConstants.timePeriod.LASTDAY, msecs: 24*60*60*1000, overview: 15, spark: 120}
                        ,{name: self.apmConstants.timePeriod.LAST2DAYS, msecs: 2*24*60*60*1000, overview: 30, spark: 240}
                        ,{name: self.apmConstants.timePeriod.LAST3DAYS, msecs: 3*24*60*60*1000, overview: 45, spark: 360}
                        ,{name: self.apmConstants.timePeriod.LAST4DAYS, msecs: 4*24*60*60*1000, overview: 60, spark: 480}
                        ,{name: self.apmConstants.timePeriod.LAST5DAYS, msecs: 5*24*60*60*1000, overview: 60, spark: 600}
                        ,{name: self.apmConstants.timePeriod.LAST6DAYS, msecs: 6*24*60*60*1000, overview: 60, spark: 720}
                        ,{name: self.apmConstants.timePeriod.LAST7DAYS, msecs: 7*24*60*60*1000, overview: 60, spark: 720}
                        ,{name: self.apmConstants.timePeriod.LASTWEEK, msecs: 7*24*60*60*1000, overview: 60, spark: 720}
                      ];
            ...
            self.getDateTimeConverter = function(timeRange)
            {
                ...
                if (timeRange > self.getMillisecondsFromTimePeriodName(self.apmConstants.timePeriod.LASTDAY)*2)
                {
                ...
                else if (timeRange >= self.getMillisecondsFromTimePeriodName(self.apmConstants.timePeriod.LASTDAY))
                {
            ...
            */ 
            ...
            self.validateTimePeriod = function (since, until )
            {
                ...
                if (since !== undefined && until !== undefined)
                {
                    ...
                    // checkk if time-window specified is larger than self.apmConstants.MAX_TIME_WINDOW + fudge
                    // the 'fudge' accounts for snapping of time, so if the user chose self.apmConstants.MAX_TIME_WINDOW in the timepicker, they'll
                    // end up with snapped time which is larger than self.apmConstants.MAX_TIME_WINDOW and could have been bookmarked, so we do not want to
                    // give errors for something we produced.
                    // todo: 'fudge' needs to be determined, for now doing 24hours
                    if ( until - since > self.apmConstants.MAX_TIME_WINDOW + 24*60*60*1000 )
                    {
                        logMsg = "time ignored: since - until > self.apmConstants.MAX_TIME_WINDOW";
                        return false;
                    }
                }
